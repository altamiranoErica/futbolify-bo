...
public interface TrackRepository extends CrudRepository<Track, UUID> {
    ...
    @Query("SELECT t FROM Poll p INNER JOIN p.tracks t WHERE p.pollID = ?1 ORDER BY t.numberOfVotes DESC")
    List<Track> findTracksByPoll_PollIDOrderByNumberOfVotes(UUID pollID);
}
